...
SELECT cast(100000000000 + av.id as bigint) as id,
       ...
       CASE WHEN (
                 select count(name) from account_move_line where name=av.number
            ) >= 1
            THEN (
                 select -1 * (sum(credit) - sum(debit))
                 from account_move_line where name =av.number
            )
            ELSE 0 END as total,
       CASE WHEN (days_due BETWEEN 01 AND  30) and (
                 select count(name) from account_move_line where name=av.number
            ) >= 1
            THEN (
                 select -1 * (sum(credit) - sum(debit))
       ...
       CASE WHEN (days_due BETWEEN 31 AND  60) and (
                 select count(name)
       ...
       CASE WHEN (days_due BETWEEN 61 AND  90) and (
                 select count(name)
                 from account_move_line
                 where name=av.number
            ) >= 1
       ...
       CASE WHEN (days_due BETWEEN 91 AND  120) and (
                 select count(name)
                 from account_move_line
                 where name=av.number
            ) >= 1
...
INNER JOIN
    (
        SELECT id,
               EXTRACT(
                    DAY FROM (now() - (aml2.date + INTERVAL '30 days'))
                ) AS days_due
...
WHERE av.partner_id = rp.id
        ...
        and av.move_id in (
             select move_id
             from account_move_line
             where reconcile_id is null and
                   account_id in (
                        select id
