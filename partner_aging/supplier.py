    ...
    def init(self, cr):
        ...
        SELECT
            ...
            CASE WHEN ai.id is not null
                    THEN ai.date_due ElSE l.date_maturity END as "date_due",
            days_due as "avg_days_overdue",
            res_currency.name as "currency_name",
            l.date as "oldest_invoice_date",
            CASE WHEN (l.credit - l.debit) > 0 and
                      ai.id is not null
                    THEN ai.residual
                 WHEN (l.credit - l.debit) < 0 and
                      ai.id is not null
                    THEN -1*ai.residual
                 ELSE l.credit - l.debit END as "total",
            CASE WHEN (l.credit - l.debit) > 0 and
                      (days_due BETWEEN 01 AND  30) and
                      ai.id is not null
                    THEN ai.residual
                 WHEN (l.credit - l.debit) < 0 and
                      (days_due BETWEEN 01 AND  30) and
                      ai.id is not null
                    THEN -1*ai.residual
                 WHEN (days_due BETWEEN 01 and 30) and
                      ai.id is null
                    THEN l.credit - l.debit ELSE 0 END
                    AS "days_due_01to30",
            CASE WHEN (l.credit - l.debit) > 0 and
                      (days_due BETWEEN 31 AND  60) and
                      ai.id is not null
                    THEN ai.residual
                 WHEN (l.credit - l.debit) < 0 and
                      (days_due BETWEEN 31 AND  60) and
                      ai.id is not null
                    THEN -1*ai.residual
                 WHEN (days_due BETWEEN 31 and 60) and
                      ai.id is null
                    THEN l.credit - l.debit ELSE 0 END
                    AS "days_due_31to60",
            CASE WHEN (l.credit - l.debit) > 0 and
                      (days_due BETWEEN 61 AND  90) and
                      ai.id is not null
                    THEN ai.residual
                 WHEN (l.credit - l.debit) < 0 and
                      (days_due BETWEEN 61 AND 90) and
                      ai.id is not null
                    THEN -1*ai.residual
                 WHEN (days_due BETWEEN 61 and 90) and
                      ai.id is null
                    THEN l.credit - l.debit ELSE 0 END
                    AS "days_due_61to90",
            CASE WHEN (l.credit - l.debit) > 0 and
                    ...
                    THEN ai.residual
                 WHEN (l.credit - l.debit) < 0 and
                      (days_due BETWEEN 91 AND 120) and
                      ai.id is not null
                    THEN -1*ai.residual
                 WHEN (days_due BETWEEN 91 and 120) and
                      ai.id is null
                    THEN l.credit - l.debit ELSE 0 END
                    AS "days_due_91to120",
            CASE WHEN (l.credit - l.debit) > 0 and
                 ...
                 WHEN (l.credit - l.debit) < 0 and
                      ...
                      ai.id is not null
                    THEN -1*ai.residual
                 WHEN days_due >= 121 and
                      ai.id is null
                    THEN l.credit - l.debit ELSE 0 END
                    AS "days_due_121togr",
            CASE when (l.credit - l.debit) > 0 and
                      days_due <= 0 and ai.id is not null
                    THEN ai.residual
                 WHEN (l.credit - l.debit) < 0 and
                      days_due <= 0 and ai.id is not null
                    THEN -1*ai.residual
                 WHEN days_due <= 0 and ai.id is null
                    THEN l.credit - l.debit ELSE 0 END
                    as "current",
        ...
        INNER JOIN
          (
           ...
           ELSE
            EXTRACT(DAY FROM (now() - lt.date_maturity)) END AS days_due
