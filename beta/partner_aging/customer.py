...
class lgx_account_aging_hot(osv.osv):
    ...
    def init(self, cr):
        ...
        query = """
		    ...
		    l.date as "oldest_invoice_date",
                    ...
                    CASE WHEN (days_due BETWEEN 01 AND  30) and ai.id is not null THEN ai.residual WHEN (days_due BETWEEN 01 and 30) and ai.id is null THEN l.debit - l.credit ELSE 0 END  AS "days_due_01to30",
                    CASE WHEN (days_due BETWEEN 31 AND  60) and ai.id is not null THEN ai.residual WHEN (days_due BETWEEN 31 and 60) and ai.id is null THEN l.debit - l.credit ELSE 0 END  AS "days_due_31to60",
                    CASE WHEN (days_due BETWEEN 61 AND  90) and ai.id is not null THEN ai.residual WHEN (days_due BETWEEN 61 and 90) and ai.id is null THEN l.debit - l.credit ELSE 0 END  AS "days_due_61to90",
                    CASE WHEN (days_due BETWEEN 91 AND 120) and ai.id is not null THEN ai.residual WHEN (days_due BETWEEN 91 and 120) and ai.id is null THEN l.debit - l.credit ELSE 0 END  AS "days_due_91to120",
                    CASE WHEN days_due >=121 and ai.id is not null THEN ai.residual WHEN days_due >=121 and ai.id is null THEN l.debit-l.credit ELSE 0 END AS "days_due_121togr",
		    CASE when days_due < 0 THEN 0 ELSE days_due END as "max_days_overdue",
                    CASE when days_due <= 0 and ai.id is not null THEN ai.residual WHEN days_due <=0 and ai.id is null then l.debit-l.credit ELSE 0 END as "current",
