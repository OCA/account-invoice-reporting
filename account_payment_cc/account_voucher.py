...
class account_voucher(orm.Model):
    ...
    def action_move_line_create(self, cr, uid, ids, context=None):
        base_func = super(account_voucher, self).action_move_line_create
        res = base_func(cr, uid, ids, context)
        move_line_pool = self.pool.get('account.move.line')
        move_pool = self.pool.get('account.move')

        for voucher in self.browse(cr, uid, ids):
            if not voucher.journal_id.support_creditcard_transactions:
                break
            ...
            account_payable = partner_id.property_account_payable
            account_receivable = partner_id.property_account_receivable
            ...
            account_move_id = self.account_move_get(cr, uid, voucher.id,
                                                    context=context)
            move_id = move_pool.create(cr, uid, account_move_id,
                                       context=context)

            fline = self.first_move_line_get(
                cr, uid, voucher.id, move_id, company_currency,
            ...
            if alines:
                for line in voucher.line_ids:
                    # Create one move line per voucher line where amount is not
                    # 0.0
                    if not line.amount:
                        continue
                    ...
                    move_line = {
                        ...
                        'currency_id': (
                            ...
                            (
                                company_currency !=
                                line.move_line_id.currency_id.id and
                        ...
                        'analytic_account_id': (
                            ...
                            line.account_analytic_id.id or
                            False
                        ),
                        'quantity': 1,
                        'credit': credit,
                ...
                amount = self._convert_amount(
                    cr, uid, (credit + debit), voucher.id, context=ctx
                )
                line_debit = line_credit = 0.0
    ...
    def cancel_voucher(self, cr, uid, ids, context=None):
        ...
        for voucher in self.browse(cr, uid, ids, context=context):
            ...
            if (
                ...
                cids = move_pool.search(
                    cr, uid, [("name", "=", voucher_number)], context=context
                )

                for move in move_pool.browse(cr, uid, cids, context=context):
